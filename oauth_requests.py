...
app_config = {
    ...
    "DH_GENERATOR": int(os.getenv("DH_GENERATOR")),
...
def request_token() -> requests.Response:
    return send_oauth_request(
        request_method="POST",
        request_url="https://api.ibkr.com/v1/api/oauth/request_token",
        signature_method="RSA-SHA256",
        extra_headers={
            "oauth_callback": app_config["REDIRECT_URI"],
        },
    )


def access_token(request_token: str, verifier_token: str) -> requests.Response:
    return send_oauth_request(
        request_method="POST",
        request_url="https://api.ibkr.com/v1/api/oauth/access_token",
